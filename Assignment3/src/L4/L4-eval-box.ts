...
const evalTraceExp = (exp: TraceExp): Result<void> =>
    bind(applicativeEval(exp.var, theGlobalEnv), (val: Value) =>
    mapv(applyEnvBdg(theGlobalEnv, exp.var.var), (bdg: FBinding) =>{
        const clo = getFBindingVal(bdg);
        isClosure(clo) ?
            setFBinding(bdg, makeTracedClosure(clo , exp.var.var)):
            makeFailure(`Bad procedure ${JSON.stringify(exp.var)}`)
            ;}))
...
{
    printPreTrace(proc.name, args, proc.repeat);
    proc.repeat = proc.repeat + 1;
    let result = applyClosure(proc.closure, args);
    proc.repeat = proc.repeat - 1;
    isOk(result)? printPostTrace(result.value, proc.repeat): result;
    return result;
}
